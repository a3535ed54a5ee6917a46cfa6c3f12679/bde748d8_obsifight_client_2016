...
public class CustomColorizer {
   ...
   private static int[][] blockPalettes = (int[][])((int[][])null);
   private static int[][] paletteColors = (int[][])((int[][])null);
   private static int[] skyColors = null;
   private static int[] fogColors = null;
   private static int[] underwaterColors = null;
   private static float[][][] lightMapsColorsRgb = (float[][][])((float[][][])null);
   ...
   public static void update() {
      ...
      lightMapsColorsRgb = (float[][][])((float[][][])null);
      ...
      skyColorEnd = null;
      textColors = null;
      blockPalettes = (int[][])((int[][])null);
      paletteColors = (int[][])((int[][])null);
   ...
   private static int getTextureHeight(String path, int defHeight) {
      try {
         InputStream var4 = Config.getResourceStream(new ResourceLocation(path));
         if(var4 == null) {
            return defHeight;
         } else {
            BufferedImage bi = ImageIO.read(var4);
   ...
   private static void readColorProperties(String fileName) {
      try {
         ResourceLocation var5 = new ResourceLocation(fileName);
         InputStream in = Config.getResourceStream(var5);
         if(in == null) {
            return;
         }
   ...
   private static void readCustomPalettes(Properties props, String fileName) {
      ...
      for(int var18 = 0; var18 < 256; ++var18) {
         blockPalettes[var18][0] = -1;
      }

      String var191 = "palette.block.";
      ...
      while(propNames.hasNext()) {
         String var19 = (String)propNames.next();
         name = props.getProperty(var19);
         if(var19.startsWith(var191)) {
            map.put(var19, name);
         }
      }

      String[] var201 = (String[])((String[])((String[])map.keySet().toArray(new String[map.size()])));
      paletteColors = new int[var201.length][];

      for(int var20 = 0; var20 < var201.length; ++var20) {
         name = var201[var20];
         String value = props.getProperty(name);
         Config.log("Block palette: " + name + " = " + value);
         String path = name.substring(var191.length());
         ...
         for(int ix = 0; ix < indexStrs.length; ++ix) {
            ...
            if(blockStr.contains(":")) {
               String[] var21 = Config.tokenize(blockStr, ":");
               blockStr = var21[0];
               String metadataStr = var21[1];
            ...
            int var211 = Config.parseInt(blockStr, -1);
            if(var211 >= 0 && var211 <= 255) {
               if(var211 != Block.getIdFromBlock(Blocks.grass) && var211 != Block.getIdFromBlock(Blocks.tallgrass) && var211 != Block.getIdFromBlock(Blocks.leaves) && var211 != Block.getIdFromBlock(Blocks.vine)) {
                  if(metadata == -1) {
                     blockPalettes[var211][0] = var20;
                  } else {
                     if(blockPalettes[var211].length < 16) {
                        blockPalettes[var211] = new int[16];
                        Arrays.fill(blockPalettes[var211], -1);
                     }

                     blockPalettes[var211][metadata] = var20;
                  }
               }
            } else {
               Config.log("Invalid block index: " + var211 + " in palette: " + name);
            }
         }
      }
   }
   ...
   private static int readColor(Properties props, String name) {
      ...
      if(str == null) {
         ...
         try {
            int var4 = Integer.parseInt(str, 16) & 16777215;
   ...
   private static int[] getCustomColors(String path, int length) {
      try {
         ResourceLocation var6 = new ResourceLocation(path);
         InputStream in = Config.getResourceStream(var6);
         if(in == null) {
            return null;
         } else {
            int[] colors = TextureUtil.readImageData(Config.getResourceManager(), var6);
            if(colors == null) {
               return null;
   ...
   public static int getColorMultiplier(Block block, IBlockAccess blockAccess, int x, int y, int z) {
      if(useDefaultColorMultiplier) {
         ...
         if(blockPalettes != null) {
            int useSwampColors1 = Block.getIdFromBlock(block);
            if(useSwampColors1 >= 0 && useSwampColors1 < 256) {
               int[] smoothColors1 = blockPalettes[useSwampColors1];
               boolean type2 = true;
               int type1;
               if(smoothColors1.length > 1) {
                  metadata = blockAccess.getBlockMetadata(x, y, z);
                  type1 = smoothColors1[metadata];
               } else {
                  type1 = smoothColors1[0];
               }
         ...
         boolean useSwampColors11 = Config.isSwampColors();
         boolean smoothColors11 = false;
         byte type21 = 0;
         metadata = 0;
         if(block != Blocks.grass && block != Blocks.tallgrass) {
            if(block == Blocks.leaves) {
               type21 = 2;
               smoothColors11 = Config.isSmoothBiomes();
               metadata = blockAccess.getBlockMetadata(x, y, z);
               if((metadata & 3) == 1) {
         ...
         if(smoothColors11) {
            return getSmoothColorMultiplier(block, blockAccess, x, y, z, colors, swampColors, type21, metadata);
         } else {
   ...
   private static int getSmoothColorMultiplier(Block block, IBlockAccess blockAccess, int x, int y, int z, int[] colors, int[] swampColors, int type, int metadata) {
      ...
      for(r = x - 1; r <= x + 1; ++r) {
         for(g = z - 1; g <= z + 1; ++g) {
            ...
            if(var16 == null) {
               ...
               var17 = getCustomColor(var16, blockAccess, r, y, g);
            }
      ...
      r = sumRed / 9;
      g = sumGreen / 9;
      int var171 = sumBlue / 9;
      return r << 16 | g << 8 | var171;
   }
   ...
   private static int parseColor(String str) {
      if(str == null) {
         ...
         try {
            int var2 = Integer.parseInt(str, 16) & 16777215;
            return var2;
            ...
            if(lightMapRgb == null) {
               ...
               if(nightvision && height < 64) {
                  ...
                  if(width < 16) {
                     ...
                     lightMapsColorsRgb[lightMapIndex] = (float[][])((float[][])null);
                     return false;
                  } else {
                     int startIndex = 0;
                     if(nightvision) {
                     ...
                     for(int is = 0; is < 16; ++is) {
                        for(int it = 0; it < 16; ++it) {
                           int r;
                           for(r = 0; r < 3; ++r) {
                              float var21 = Config.limitTo1(sunRgbs[is][r] + torchRgbs[it][r]);
                              if(hasGamma) {
                                 float var22 = 1.0F - var21;
                                 var22 = 1.0F - var22 * var22 * var22 * var22;
                                 var21 = gamma * var22 + (1.0F - gamma) * var21;
                              }
                           ...
                           r = (int)(rgb[0] * 255.0F);
                           int var211 = (int)(rgb[1] * 255.0F);
                           int var221 = (int)(rgb[2] * 255.0F);
                           lmColors[is * 16 + it] = -16777216 | r << 16 | var211 << 8 | var221;
                        }
                     }
   ...
   private static void getLightMapColumn(float[][] origMap, float x, int offset, int width, float[][] colRgb) {
      ...
      if(xLow == xHigh) {
         for(int dLow = 0; dLow < 16; ++dLow) {
            float[] dHigh = origMap[offset + dLow * width + xLow];
            float[] y = colRgb[dLow];

            for(int rgbLow = 0; rgbLow < 3; ++rgbLow) {
               y[rgbLow] = dHigh[rgbLow];
            }
         }
      } else {
         float var14 = 1.0F - (x - (float)xLow);
         float var15 = 1.0F - ((float)xHigh - x);

         for(int var16 = 0; var16 < 16; ++var16) {
            float[] var17 = origMap[offset + var16 * width + xLow];
            float[] rgbHigh = origMap[offset + var16 * width + xHigh];
            float[] rgb = colRgb[var16];

            for(int i = 0; i < 3; ++i) {
               rgb[i] = var17[i] * var14 + rgbHigh[i] * var15;
            }
         }
      }
   }
