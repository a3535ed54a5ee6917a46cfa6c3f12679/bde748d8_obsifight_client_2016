   ...
   private float[] parseAxis(String str, float[] defVal) {
      if(str == null) {
         ...
         if(strs.length != 3) {
            ...
            for(int var9 = 0; var9 < strs.length; ++var9) {
               fs[var9] = Config.parseFloat(strs[var9], Float.MIN_VALUE);
               if(fs[var9] == Float.MIN_VALUE) {
                  Config.warn("Invalid axis: " + str);
                  return defVal;
               }

               if(fs[var9] < -1.0F || fs[var9] > 1.0F) {
            ...
            float ay = fs[1];
            float az = fs[2];
            if(var91 * var91 + ay * ay + az * az < 1.0E-5F) {
               Config.warn("Invalid axis values: " + str);
               return defVal;
            } else {
               float[] as = new float[]{az, ay, -var91};
               return as;
            }
         }
      }
