...
public class CustomItemProperties {
   ...
   private int[] parseItems(String str, String str2) {
      ...
      if(str == null) {
         ...
         int i;
         label58:
         for(int var14 = 0; var14 < tokens.length; ++var14) {
            String var15 = tokens[var14];
            i = Config.parseInt(var15, -1);
            if(i >= 0) {
               setItemIds.add(new Integer(i));
            } else {
               int id;
               if(var15.contains("-")) {
                  String[] var16 = Config.tokenize(var15, "-");
                  if(var16.length == 2) {
                     int var17 = Config.parseInt(var16[0], -1);
                     id = Config.parseInt(var16[1], -1);
                     if(var17 >= 0 && id >= 0) {
                        int min = Math.min(var17, id);
                        int max = Math.max(var17, id);
                        int x = min;
               ...
               Object var161 = Item.itemRegistry.getObject(var15);
               if(!(var161 instanceof Item)) {
                  Config.dbg("Item not found: " + var15);
               } else {
                  Item var171 = (Item)var161;
                  id = Item.getIdFromItem(var171);
                  if(id < 0) {
                     Config.dbg("Item not found: " + var15);
                  } else {
         ...
         Integer[] var141 = (Integer[])((Integer[])((Integer[])setItemIds.toArray(new Integer[setItemIds.size()])));
         int[] var151 = new int[var141.length];

         for(i = 0; i < var151.length; ++i) {
            var151[i] = var141[i].intValue();
         }
      ...
      if(str == null) {
         ...
         if(countMinus > 1) {
            ...
            for(min = 0; min < tokens.length; ++min) {
               String var8 = tokens[min];
               int val = Config.parseInt(var8, -1);
               if(val < 0) {
                  Config.warn("Invalid range: " + str);
                  return null;
               }
            ...
            if(vals.length == 1) {
               ...
               int var81 = Math.max(vals[0], vals[1]);
               return new RangeInt(min, var81);
            } else {
               Config.warn("Invalid range: " + str);
               return null;
            }
   ...
   private NbtTagValue[] parseNbtTagValues(Properties props) {
      ...
      if(mapNbt.size() <= 0) {
         ...
         while(nbts.hasNext()) {
            String nbts1 = (String)nbts.next();
            String val = (String)mapNbt.get(nbts1);
            NbtTagValue nbt = new NbtTagValue(nbts1, val);
            listNbts.add(nbt);
         }

         NbtTagValue[] nbts11 = (NbtTagValue[])((NbtTagValue[])((NbtTagValue[])listNbts.toArray(new NbtTagValue[listNbts.size()])));
         return nbts11;
      }
   }
