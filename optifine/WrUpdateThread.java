...
public class WrUpdateThread extends Thread {
   ...
   public void run() {
      ...
      WrUpdateThread.ThreadUpdateListener updateListener = new WrUpdateThread.ThreadUpdateListener((WrUpdateThread.NamelessClass750854124)null);

      while(!Thread.interrupted() && !this.terminated) {
         try {
            WorldRendererThreaded var9 = this.getRendererToUpdate();
            if(var9 == null) {
               return;
            }
            ...
            try {
               this.currentRenderer = var9;
               Tessellator.instance = this.threadTessellator;
               var9.updateRenderer(updateListener);
            } finally {
               Tessellator.instance = this.mainTessellator;
            }

            this.rendererUpdated(var9);
   ...
   private WorldRendererThreaded getRendererToUpdate() {
      ...
      synchronized(this.lock) {
         WorldRendererThreaded wrt;
         while(this.updateList.size() <= 0) {
            try {
               this.lock.wait(2000L);
               if(this.terminated) {
                  wrt = null;
                  WorldRendererThreaded var10000 = (WorldRendererThreaded)wrt;
                  return var10000;
               }
         ...
         wrt = (WorldRendererThreaded)this.updateList.remove(0);
         this.lock.notifyAll();
         return wrt;
      }
   }
   ...
   private class ThreadUpdateListener implements IWrUpdateListener {
      ...
      private ThreadUpdateListener() {
         WrUpdateThread.ThreadUpdateControl var10001 = new WrUpdateThread.ThreadUpdateControl;
         WrUpdateThread.this.getClass();
         var10001.<init>((WrUpdateThread.NamelessClass750854124)null);
         this.tuc = var10001;
      }
