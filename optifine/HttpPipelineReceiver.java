...
public class HttpPipelineReceiver extends Thread {
   ...
   public void run() {
      while(!Thread.interrupted()) {
         ...
         try {
            ...
            InputStream var5 = this.httpPipelineConnection.getInputStream();
            HttpResponse resp = this.readResponse(var5);
            this.httpPipelineConnection.onResponseReceived(currentRequest, resp);
         } catch (InterruptedException var4) {
            return;
         } catch (Exception var51) {
            this.httpPipelineConnection.onExceptionReceive(currentRequest, var51);
         }
      }
   }
   ...
   private HttpResponse readResponse(InputStream in) throws IOException {
      ...
      if(parts.length < 3) {
         ...
         while(true) {
            ...
            if(body.length() <= 0) {
               byte[] lenStr1 = null;
               val = (String)headers.get("Content-Length");
               if(val != null) {
                  int enc1 = Config.parseInt(val, -1);
                  if(enc1 > 0) {
                     lenStr1 = new byte[enc1];
                     this.readFull(lenStr1, in);
                  }
               } else {
                  enc = (String)headers.get("Transfer-Encoding");
                  if(Config.equals(enc, "chunked")) {
                     lenStr1 = this.readContentChunked(in);
                  }
               }

               return new HttpResponse(status, statusLine, headers, lenStr1);
            }

            int lenStr = body.indexOf(":");
            if(lenStr > 0) {
               enc = body.substring(0, lenStr).trim();
               val = body.substring(lenStr + 1).trim();
               headers.put(enc, val);
            }
         }
      }
   ...
   private String readLine(InputStream in) throws IOException {
      ...
      byte[] bytes11 = baos.toByteArray();
      String str = new String(bytes11, ASCII);
      if(hasCRLF) {
