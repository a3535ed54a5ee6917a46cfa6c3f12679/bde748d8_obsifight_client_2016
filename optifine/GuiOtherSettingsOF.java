...
public class GuiOtherSettingsOF extends GuiScreen implements GuiYesNoCallback {
   ...
   protected void actionPerformed(GuiButton guibutton) {
      if(guibutton.enabled) {
         ...
         if(guibutton.id == 210) {
            mc.gameSettings.saveOptions();
            GuiYesNo scaledresolution1 = new GuiYesNo(this, "Reset all video settings to their default values?", "", 9999);
            mc.displayGuiScreen(scaledresolution1);
         }

         if(guibutton.id != GameSettings.Options.CLOUD_HEIGHT.ordinal()) {
            ScaledResolution scaledresolution11 = new ScaledResolution(mc, mc.displayWidth, mc.displayHeight);
            int i = scaledresolution11.getScaledWidth();
            int j = scaledresolution11.getScaledHeight();
