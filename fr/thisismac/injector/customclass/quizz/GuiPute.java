...
public class GuiPute extends GuiScreen {
   ...
   public GuiPute(Minecraft minecraft, World theWorld) {
      mc = minecraft;
      this.world = theWorld;
      this.xSize = 256;
      this.ySize = 256;
   ...
   public void drawScreen(int i, int j, float f) {
      ...
      drawRect(this.width / 2 - 100, this.height - 27, this.width / 2 - 100 + 0, this.height - 16, -267739136);
      this.drawCenteredString(this.fontRendererObj, ChatFormatting.GRAY + "Progression", this.width / 2, this.height - 40, 16777215);
      this.drawCenteredString(this.fontRendererObj, ChatFormatting.GREEN + "0%", this.width / 2, this.height - 13, 16777215);
   ...
   protected void drawBackgroundImage() {
      ...
      this.drawString(this.fontRendererObj, "Quel effect donne le cookie garnet ?", this.width / 2 - 85, this.height / 4 + 20 + 0, 10526880);
   ...
   protected void actionPerformed(GuiButton guibutton) {
      if(guibutton.enabled) {
         if(guibutton.id == 2) {
            ++Calculator.points;
            Calculator.question1 = true;
            mc.displayGuiScreen(new Question2(mc, mc.theWorld));
         }

         if(guibutton.id == 3 || guibutton.id == 4 || guibutton.id == 5) {
            Calculator.question1 = false;
            mc.displayGuiScreen(new Question2(mc, mc.theWorld));
         }
      }
   }
