...
import fr.thisismac.injector.customclass.craft.GuiWorkSurface;
...
public class EntityPlayerSP extends AbstractClientPlayer {
   ...
   public void onLivingUpdate() {
      ...
      if(this.mc.playerController.enableEverythingIsScrewedUpMode()) {
         ...
         this.wasSneaking = this.movementInput.sneak;
         this.isMovingForward = this.movementInput.moveForward >= 0.8F;
         CustomMovementInput.update(this.mc, (MovementInputFromOptions)this.movementInput);
         ...
         if(ClientModManager.isModDisabled()) {
            if(this.onGround && !this.isMovingForward && this.movementInput.moveForward >= 0.8F && !this.isSprinting() && enoughHunger && !this.isUsingItem() && !this.isPotionActive(Potion.blindness)) {
            ...
            if(!this.isSprinting() && this.movementInput.moveForward >= 0.8F && enoughHunger && !this.isUsingItem() && !this.isPotionActive(Potion.blindness) && this.mc.gameSettings.keyBindSprint.getIsKeyPressed()) {
               this.setSprinting(true);
            }
         } else {
            if(this.mc.currentScreen != null && this.isSneaking() && CustomMovementInput.sneakToggle && !(this.mc.currentScreen instanceof GuiGameOver)) {
               this.shouldRestoreSneakToggle = true;
               CustomMovementInput.sneakToggle = false;
            }

            if(this.shouldRestoreSneakToggle && this.mc.currentScreen == null) {
            ...
            if((dblTap && !this.isSprinting() || !dblTap) && this.onGround && enoughHunger && !this.isUsingItem() && !this.isPotionActive(Potion.blindness)) {
               this.setSprinting(sprint);
            }

            CustomMovementInput.held = sprint;
            if(dblTap && !CustomMovementInput.held && CustomMovementInput.stoptime == 0 && this.onGround && !this.isMovingForward && this.movementInput.moveForward >= 0.8F && !this.isSprinting() && enoughHunger && !this.isUsingItem() && !this.isPotionActive(Potion.blindness)) {
               if(this.sprintToggleTimer == 0) {
                  this.sprintToggleTimer = 7;
               } else {
                  this.setSprinting(true);
            ...
            if(ClientSettings.flySpeedBoost > 0) {
               if(sprint && this.capabilities.isFlying && ClientModManager.canBoostFlying(this.mc)) {
                  this.capabilities.setFlySpeed(0.05F * (float)(1 + ClientSettings.flySpeedBoost));
                  if(this.movementInput.sneak) {
                     this.motionY -= 0.15D * (double)ClientSettings.flySpeedBoost;
                  }
         ...
         if(ClientModManager.keyBindOptionsMenu.getIsKeyPressed()) {
            this.mc.displayGuiScreen(new GuiSprint((GuiScreen)null));
         }

         if(this.isSprinting() && this.isSneaking() && !this.capabilities.isFlying) {
            this.setSprinting(false);
         }

         if(this.isSprinting() && (this.movementInput.moveForward < 0.8F || this.isCollidedHorizontally || !enoughHunger) && (!ClientModManager.canRunInAllDirs(this.mc) || !ClientSettings.enableAllDirs || this.movementInput.moveForward == 0.0F && this.movementInput.moveStrafe == 0.0F)) {
            this.setSprinting(false);
         }
   ...
   public float getFOVMultiplier() {
      ...
      if(this.isUsingItem() && (this.getItemInUse().getItem() == Items.bow || this.getItemInUse().getItem() instanceof CustomBow)) {
   ...
   public void displayWork(int p_71058_1_, int p_71058_2_, int p_71058_3_) {
      this.mc.displayGuiScreen(new GuiWorkSurface(this.inventory, this.worldObj, p_71058_1_, p_71058_2_, p_71058_3_));
   }
