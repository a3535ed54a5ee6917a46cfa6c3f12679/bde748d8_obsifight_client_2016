      ...
      if(!par2 && !par3) {
         ...
         if(player == null || player.chunkCoordX != this.lastPlayerChunkX || player.chunkCoordZ != this.lastPlayerChunkZ || this.eligibleChunksForSpawning.size() <= 0) {
            ...
            for(var5 = 0; var5 < par1WorldServer.playerEntities.size(); ++var5) {
               EntityPlayer var411 = (EntityPlayer)par1WorldServer.playerEntities.get(var5);
               int var42 = MathHelper.floor_double(var411.posX / 16.0D);
               var8 = MathHelper.floor_double(var411.posZ / 16.0D);
               byte var43 = 8;

               for(int var44 = -var43; var44 <= var43; ++var44) {
                  for(countEntities = -var43; countEntities <= var43; ++countEntities) {
                     var45 = var44 == -var43 || var44 == var43 || countEntities == -var43 || countEntities == var43;
                     var39 = new ChunkCoordIntPair(var44 + var42, countEntities + var8);
                     if(!var45) {
         ...
         var5 = 0;
         ChunkCoordinates var421 = par1WorldServer.getSpawnPoint();
         EnumCreatureType[] var431 = EnumCreatureType.values();
         var8 = var431.length;

         for(int var441 = 0; var441 < var8; ++var441) {
            EnumCreatureType var451 = var431[var441];
            var45 = false;
            if(Reflector.ForgeWorld_countEntities.exists()) {
               countEntities = Reflector.callInt(par1WorldServer, Reflector.ForgeWorld_countEntities, new Object[]{var451, Boolean.valueOf(true)});
            } else {
               countEntities = par1WorldServer.countEntities(var451.getCreatureClass());
            }

            if((!var451.getPeacefulCreature() || par3) && (var451.getPeacefulCreature() || par2) && (!var451.getAnimal() || par4) && countEntities <= var451.getMaxNumberOfCreature() * this.eligibleChunksForSpawning.size() / 256) {
               ...
               while(var46.hasNext()) {
                  ...
                  if(!((Boolean)this.eligibleChunksForSpawning.get(var39)).booleanValue()) {
                     ...
                     if(!par1WorldServer.getBlock(var14, var15, var16).isNormalCube() && par1WorldServer.getBlock(var14, var15, var16).getMaterial() == var451.getCreatureMaterial()) {
                        int var17 = 0;
                        int var18 = 0;
                           ...
                           while(true) {
                              if(var25 < 4) {
                                 label135: {
                                    ...
                                    if(canCreatureTypeSpawnAtLocation(var451, par1WorldServer, var19, var20, var21)) {
                                       ...
                                       if(par1WorldServer.getClosestPlayer((double)var26, (double)var27, (double)var28, 24.0D) == null) {
                                          float var29 = var26 - (float)var421.posX;
                                          float var30 = var27 - (float)var421.posY;
                                          float var31 = var28 - (float)var421.posZ;
                                          float var32 = var29 * var29 + var30 * var30 + var31 * var31;
                                          if(var32 >= 576.0F) {
                                             if(var23 == null) {
                                                var23 = par1WorldServer.spawnRandomCreature(var451, var19, var20, var21);
                                                if(var23 == null) {
                                                   break label135;
                                                }
                                             }
